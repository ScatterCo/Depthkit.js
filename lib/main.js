...
class DracoMeshSequencePlayer extends THREE.Object3D {
        ...
        if ('requestVideoFrameCallback' in HTMLVideoElement.prototype) {
            const onNewVideoFrame = (now, metadata) => {
                ...
                if (this.currentFrame === this.previousFrame) {
                    ...
                    if (!this.video.paused && this.meshReadyState < HTMLMediaElement.HAVE_FUTURE_DATA) {
    ...
    loadMesh(frame) {
        ...
        this.dracoLoader.load(
            ...
            (geometry) => {
                ...
                if (this.meshReadyState >= HTMLMediaElement.HAVE_FUTURE_DATA) {
                    this.updateMesh(this.currentFrame);
                    if (this.video.paused && this.bufferingMeshes) {
    ...
    updateReadyState() {
        ...
        let meshesLoaded = this.numBufferedGeometryFrames(this.currentFrame);
        if (meshesLoaded >= 1) {
